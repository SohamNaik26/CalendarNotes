        ...
        return Future { [weak self] promise in
            self?.backgroundContext.perform {
                do {
                    ...
                    let event = CalendarEvent(context: context, title: title, startDate: startDate, endDate: endDate, category: category, location: location, notes: notes, isRecurring: isRecurring, recurrenceRule: recurrenceRule)
    ...
    func createNoteAsync(content: String, linkedDate: Date? = nil, tags: String? = nil) -> AnyPublisher<Void, Error> {
        return Future { [weak self] promise in
            self?.backgroundContext.perform {
                do {
                    let context = self?.backgroundContext ?? NSManagedObjectContext(concurrencyType: .privateQueueConcurrencyType)
                    let note = Note(context: context, content: content, linkedDate: linkedDate, tags: tags)
