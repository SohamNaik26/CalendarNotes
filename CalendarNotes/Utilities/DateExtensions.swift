...
extension Calendar {
    /// Returns an array of 42 dates (6 weeks) for a calendar grid including previous and next month overflow
    func getDaysInMonth(for date: Date) -> [Date] {
        // Get the first day of the month
        guard let monthStart = self.date(from: self.dateComponents([.year, .month], from: date)),
              let monthRange = self.range(of: .day, in: .month, for: date) else {
            return []
        }
        ...
        // Generate array of dates starting from previous month overflow
        var dates: [Date] = []
        
        // Add previous month days
        if daysFromPreviousMonth > 0 {
            if let previousMonthStart = self.date(byAdding: .day, value: -daysFromPreviousMonth, to: monthStart) {
                for day in 0..<daysFromPreviousMonth {
                    if let date = self.date(byAdding: .day, value: day, to: previousMonthStart) {
                        dates.append(date)
                    }
                }
            }
        ...
        for day in monthRange {
            if let date = self.date(byAdding: .day, value: day - 1, to: monthStart) {
                dates.append(date)
            }
        }
        ...
        if let monthEnd = dates.last {
            for day in 1...remainingDays {
                if let date = self.date(byAdding: .day, value: day, to: monthEnd) {
                    dates.append(date)
                }
            }
        }
