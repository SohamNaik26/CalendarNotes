        ...
        HStack(spacing: 0) {
            ForEach(Array(tabs.enumerated()), id: \.offset) { index, tab in
                Button(action: {
                    ...
                    VStack(spacing: ScreenSize.isSmallDevice ? 4 : 6) {
                        Image(systemName: tab.icon)
                            .font(ScreenSize.isSmallDevice ? .system(size: 18) : .title3)
                            .foregroundColor(selectedTab == index ? .cnPrimary : .secondary)
                            .scaleEffect(selectedTab == index ? 1.1 : 1.0)
                        
                        // Show labels on medium, large, and extra large devices
                    ...
                    .padding(.vertical, ScreenSize.isSmallDevice ? 8 : 10)
                }
                .buttonStyle(.plain)
            }
        }
