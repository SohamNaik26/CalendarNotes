...
class ScheduledCleanupService: ObservableObject {
    ...
    private func performWeeklyCleanup() async throws {
        ...
        // Smart: auto-archive old bookmarks (120 days)
        try SmartBookmarkService.shared.autoArchiveOldBookmarks(olderThan: 120)
        
        // Smart: broken link sweep (report only)
        let ctx = coreDataManager.viewContext
        let req: NSFetchRequest<Bookmark> = Bookmark.fetchRequest()
        let all = (try? ctx.fetch(req)) ?? []
        let broken = await SmartBookmarkService.shared.checkBrokenLinks(for: all)
        if !broken.isEmpty { print("⚠️ Broken links detected: \(broken.count)") }
