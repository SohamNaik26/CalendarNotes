...
class NotificationManager: ObservableObject {
    ...
    #if DEBUG
    // Lazy in DEBUG to avoid crash - only accessed when actually needed
    // Since we skip all notification setup in DEBUG init, this should never be accessed
    private lazy var notificationCenter: UNUserNotificationCenter = UNUserNotificationCenter.current()
    #else
    ...
    private init() {
        #if DEBUG
        // Skip notification setup in DEBUG mode to avoid crashes
        // Skip setupNotificationCategories, checkAuthorizationStatus, and loadPendingNotifications
        #else
