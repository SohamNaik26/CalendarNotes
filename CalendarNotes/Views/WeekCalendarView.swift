...
struct AllDayEventCard: View {
    ...
    var body: some View {
        HStack(spacing: AppSpacing.small) {
            RoundedRectangle(cornerRadius: 2)
                .fill(categoryColor)
                .frame(width: ScreenSize.isSmallDevice ? 2 : 3)
        ...
        .padding(.vertical, ScreenSize.isSmallDevice ? 4 : 6)
    ...
    private var cardSpacing: CGFloat {
        ScreenSize.isSmallDevice ? 2 : 4
    }
    
    private var cardTitleFont: Font {
        ScreenSize.isSmallDevice ? AppFonts.caption : AppFonts.subheadline
    }
...
struct EventDetailView: View {
    ...
    @State private var linkedVoiceNotes: [VoiceNote] = []
    @State private var showingVoiceRecorder = false
    @State private var playingVoiceNoteId: UUID?
    ...
    var body: some View {
        NavigationView {
            List {
                ...
                if !event.agendaItems.isEmpty {
                    Section("Agenda") {
                        ForEach(event.agendaItems, id: \.self) { item in
                ...
                if !event.preparationChecklist.isEmpty {
                    Section("Preparation") {
                        ForEach(event.preparationChecklist, id: \.self) { item in
                ...
                if !recommendedBookmarks.isEmpty || !recommendedNotes.isEmpty {
                        ...
                        if !recommendedBookmarks.isEmpty {
                            VStack(alignment: .leading, spacing: 6) {
                                Text("Bookmarks")
                                    .font(.subheadline)
                                    .fontWeight(.semibold)
                                ForEach(recommendedBookmarks, id: \.objectID) { bookmark in
                                    Label(bookmark.title ?? bookmark.url ?? "Bookmark", systemImage: "bookmark")
                                        .font(.body)
                        ...
                        if !recommendedNotes.isEmpty {
                            VStack(alignment: .leading, spacing: 6) {
                                Text("Notes")
                                    .font(.subheadline)
                                    .fontWeight(.semibold)
                                ForEach(recommendedNotes, id: \.objectID) { note in
                                    Label(note.summaryText ?? (note.content ?? "Note"), systemImage: "doc.text")
                                        .font(.body)
                                        .lineLimit(2)
                                }
                            }
            ...
            .sheet(isPresented: $showingVoiceRecorder) {
                VoiceNoteRecordingSheet(event: event, onSave: {
                    loadLinkedVoiceNotes()
                })
            }
            .onAppear {
                loadLinkedVoiceNotes()
    ...
    private var voiceNotesSection: some View {
        Section {
            ...
            if linkedVoiceNotes.isEmpty {
                Text("No voice notes attached")
                    .font(.caption)
                    .foregroundColor(.secondary)
                    .padding(.vertical, 4)
            } else {
                ForEach(linkedVoiceNotes) { voiceNote in
                    VoiceNoteRowCompact(
                        note: voiceNote,
                        isPlaying: playingVoiceNoteId == voiceNote.id,
                        onPlay: {
                            if playingVoiceNoteId == voiceNote.id {
                                playingVoiceNoteId = nil
                            } else {
                                playingVoiceNoteId = voiceNote.id
                            }
                        }
                    )
                }
    ...
    private func loadLinkedVoiceNotes() {
        do {
            linkedVoiceNotes = try VoiceNoteRepository.shared.fetchVoiceNotes(for: .event(event))
        } catch {
                ...
                HStack(spacing: 8) {
                    ...
                    VStack(alignment: .leading, spacing: 2) {
                        Text(event.title ?? "Untitled Event")
                            .font(.title3)
                            .fontWeight(.bold)
                        Text(event.eventKind.displayName)
                            .font(.subheadline)
                            .foregroundColor(.secondary)
                    }
                    Spacer()
            ...
            HStack {
                ...
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    if event.durationMinutes > 0 {
                        Text("Duration: \(event.durationDescription)")
                            .font(.caption)
                            .foregroundColor(.secondary)
                    }
                }
