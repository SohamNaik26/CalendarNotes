...
class CloudKitNotificationManager: ObservableObject {
    ...
    #if DEBUG
    // Lazy in DEBUG so the app does not touch UNUserNotificationCenter during startup
    // This avoids crashes when the bundle identifier is not yet available
    ...
    private init() {
        #if DEBUG
        // Skip registering observers in DEBUG to prevent early notification center access
        #else
