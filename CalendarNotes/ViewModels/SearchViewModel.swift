...
class SearchViewModel: ObservableObject {
    ...
    @Published var bookmarkResults: [Bookmark] = []
    ...
    var hasResults: Bool {
        !eventResults.isEmpty || !noteResults.isEmpty || !taskResults.isEmpty || !bookmarkResults.isEmpty
    }
    
    var totalResultCount: Int {
        eventResults.count + noteResults.count + taskResults.count + bookmarkResults.count
    }
    ...
    var allResults: [SearchResult] {
        ...
        results += bookmarkResults.map { bookmark in
            SearchResult(
                ...
                subtitle: bookmark.bookmarkDescription ?? bookmark.url,
                date: bookmark.createdDate,
                category: bookmark.contentTypeValue.rawValue.capitalized,
                isCompleted: nil,
                object: bookmark
            )
        }
    ...
    private func performSearch(query: String) {
        ...
        Task { @MainActor in
            do {
                ...
                async let bookmarks = searchBookmarks(query: query)
                
                (eventResults, noteResults, taskResults, bookmarkResults) = try await (events, notes, tasks, bookmarks)
    ...
    private func searchBookmarks(query: String) async throws -> [Bookmark] {
        let request: NSFetchRequest<Bookmark> = Bookmark.fetchRequest()
        var subpredicates: [NSPredicate] = []
        subpredicates.append(NSPredicate(format: "title CONTAINS[cd] %@", query))
        subpredicates.append(NSPredicate(format: "bookmarkDescription CONTAINS[cd] %@", query))
        subpredicates.append(NSPredicate(format: "url CONTAINS[cd] %@", query))
        subpredicates.append(NSPredicate(format: "notes CONTAINS[cd] %@", query))
        subpredicates.append(NSPredicate(format: "tags CONTAINS[cd] %@", query))
        
        var predicates: [NSPredicate] = [NSCompoundPredicate(orPredicateWithSubpredicates: subpredicates)]
        ...
        request.predicate = NSCompoundPredicate(andPredicateWithSubpredicates: predicates)
        request.sortDescriptors = [NSSortDescriptor(keyPath: \Bookmark.createdDate, ascending: false)]
        request.fetchLimit = 50
