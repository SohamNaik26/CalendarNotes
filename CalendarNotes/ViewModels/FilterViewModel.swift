...
class FilterViewModel: ObservableObject {
    ...
    @Published var showScheduledReminders: Bool = true
    @Published var showBirthdays: Bool = true
    @Published var showHolidays: Bool = true
    @Published var showSiriSuggestions: Bool = true
    ...
    var activeFilterCount: Int {
        ...
        if !showScheduledReminders || !showBirthdays || !showHolidays || !showSiriSuggestions {
            count += 1
        }
    ...
    var calendarFilterOptions: CalendarFilterOptions {
        CalendarFilterOptions(
            showScheduledReminders: showScheduledReminders,
            showBirthdays: showBirthdays,
            showHolidays: showHolidays,
            showSiriSuggestions: showSiriSuggestions
        )
    }
    ...
    func filterEvents(_ events: [CalendarEvent]) -> [CalendarEvent] {
        ...
        let loweredType: (CalendarEvent) -> String = { event in
            event.eventType?.lowercased() ?? ""
        }

        if !showScheduledReminders {
            filtered = filtered.filter { event in
                let type = loweredType(event)
                return !type.contains("reminder")
            }
        }
        ...
        if !showHolidays {
            filtered = filtered.filter { event in
                ...
                return !(type.contains("holiday") || category.contains("holiday") || title.contains("holiday"))
            }
        }

        if !showSiriSuggestions {
            filtered = filtered.filter { event in
                let type = loweredType(event)
                return !type.contains("siri")
            }
        }
