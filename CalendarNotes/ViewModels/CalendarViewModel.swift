...
class CalendarViewModel: ObservableObject {
    ...
    var currentWeekDates: [Date] {
        ...
        while date < weekInterval.end {
            ...
            guard let nextDate = calendar.date(byAdding: .day, value: 1, to: date) else {
                // If date calculation fails, break to avoid infinite loop
                break
            }
            date = nextDate
        }
    ...
    private func setupObservers() {
        ...
        // Reload events when current month changes (with debounce to prevent rapid fetches)
        $currentMonth
    ...
    private func performLoad() async {
        ...
        await fetchEvents() // Today's events are critical
    ...
    private func fetchEvents() async {
        ...
        // Check if we already have data for this range (with some overlap tolerance)
        if let lastRange = lastFetchRange,
           startDate >= lastRange.start && endDate <= lastRange.end {
            // We already have data for this range, skip fetch
            return
        }
        ...
        do {
            // Fetch on background thread with timeout
            let fetched = try await NetworkTimeoutHandler.shared.withTimeout(timeout: 10.0) {
                try await TimeProfiler.shared.measureAsync("CalendarViewModel.fetchEvents") {
                    try await self.manager.fetchAsync(self.createFetchRequest(from: startDate, to: endDate))
                }
            }
            await MainActor.run {
                events = fetched
                eventsState = .loaded(fetched)
                isLoading = false
                lastFetchRange = (startDate, endDate)
            }
        } catch {
            await MainActor.run {
                let cachedEvents = eventsState.value ?? []
    ...
    private func fetchTasks() async {
        ...
        request.fetchLimit = 500 // Limit tasks loaded
        do {
            // Fetch on background thread with timeout
            let fetched = try await NetworkTimeoutHandler.shared.withTimeout(timeout: 10.0) {
                try await TimeProfiler.shared.measureAsync("CalendarViewModel.fetchTasks") {
                    try await self.manager.fetchAsync(request)
                }
            }
            await MainActor.run {
                tasks = fetched
                tasksState = .loaded(fetched)
            }
        } catch {
            await MainActor.run {
                let cachedTasks = tasksState.value ?? []
                // Don't treat CancellationError as a real error (it's expected when tasks are cancelled)
                if error is CancellationError {
                    return
                }
                tasksState = .error(error)
    ...
    private func getDateRange() -> (start: Date, end: Date) {
        ...
        case .month:
            // Fetch 3 months at once (previous, current, next) for better performance when scrolling
            let start = calendar.date(byAdding: .month, value: -1, to: currentMonth.startOfMonth()) ?? currentMonth.startOfMonth()
            let end = calendar.date(byAdding: .month, value: 2, to: currentMonth.startOfMonth()) ?? currentMonth.endOfMonth()
            return (start, end)
            
        case .year:
            let startOfYear = calendar.dateInterval(of: .year, for: currentDate)?.start ?? currentDate
            let endOfYear = calendar.date(byAdding: .year, value: 1, to: startOfYear) ?? startOfYear.addingTimeInterval(31536000)
    ...
    private func createFetchRequest(from startDate: Date, to endDate: Date) -> NSFetchRequest<CalendarEvent> {
        ...
        request.relationshipKeyPathsForPrefetching = ["category", "notes"]
    ...
    func changeMonth(by value: Int) {
        if let newMonth = calendar.date(byAdding: .month, value: value, to: currentMonth) {
            ...
            // Don't call loadEvents() here - let the debounced observer handle it
        }
    }
